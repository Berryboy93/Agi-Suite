#!/usr/bin/env bash
#==============================================================================
# Agi-Suite — Full Stack Master Script v12
# Fixes: Grace period after port kill, kills all node processes, longer wait
#==============================================================================
...
deploy_api() {
  ...
  # Aggressive cleanup: kill ALL node processes on API port
  kill_port_processes "$API_PORT" "API server"

  # Also kill any lingering node processes that might hold the port
  local lingering
  lingering=$(pgrep -f "node.*index.mjs" 2>/dev/null || true)
  if [[ -n "$lingering" ]]; then
    for pid in $lingering; do
      log_warn "Killing lingering node process (PID: $pid)"
      kill -9 "$pid" 2>/dev/null || true
    done
  fi

  # Grace period: let kernel release sockets (TIME_WAIT, etc.)
  log_info "Waiting for kernel to release sockets..."
  sleep 3

  # Double-check port is truly free
  if ! port_is_free "$API_PORT"; then
    log_warn "Port still not free, waiting more..."
    sleep 3
    if ! port_is_free "$API_PORT"; then
      die "Port $API_PORT still in use after extended cleanup"
    fi
  fi
