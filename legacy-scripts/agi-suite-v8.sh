#!/usr/bin/env bash
#==============================================================================
# Agi-Suite — Full Stack Master Script v8
# Expert implementation: Fixed DB URL parsing, password reset, bulletproof deploy
# Triple-checked for all known bugs
#==============================================================================
...
# ── Database URL Parsing (Pure Bash — No External Tools) ───────────────────
parse_db_url() {
  ...
  DB_HOST="localhost"
  DB_NAME=""
  # Preserve original DB_PORT, only override if found in URL
  ...
  if [[ "$rest" == *"@"* ]]; then
    ...
    # Split creds at : to get user and pass
    if [[ "$creds" == *":"* ]]; then
      DB_USER="${creds%%:*}"
      DB_PASS="${creds#*:}"
    else
    ...
    # Split hostpart at / to separate host:port from database
    if [[ "$hostpart" == *"/"* ]]; then
    ...
    # Split host_port at : to get host and port
    if [[ "$host_port" == *":"* ]]; then
      DB_HOST="${host_port%%:*}"
  ...
  # Only override DB_PORT if we extracted a valid port
...
db_provision() {
  ...
  # Determine how to connect as superuser
  local psql_superuser=""

  # Try 1: Direct connection as postgres user (if pg_hba allows trust)
  if psql "postgresql://postgres@localhost:$DB_PORT/postgres" -c "SELECT 1;" &>/dev/null; then
    psql_superuser="psql postgresql://postgres@localhost:$DB_PORT/postgres"
  # Try 2: sudo psql
  elif sudo -n psql "postgresql://postgres@localhost:$DB_PORT/postgres" -c "SELECT 1;" &>/dev/null; then
    psql_superuser="sudo psql postgresql://postgres@localhost:$DB_PORT/postgres"
  # Try 3: peer auth as postgres OS user
  elif sudo -n -u postgres psql -c "SELECT 1;" &>/dev/null; then
    psql_superuser="sudo -u postgres psql"
  else
    die "Cannot connect as postgres superuser. Check pg_hba.conf or run: sudo -u postgres psql"
  fi

  log_db "Using superuser: $psql_superuser"

  # Create user if not exists (or reset password if exists)
  log_db "Ensuring user '$DB_USER' exists with correct password..."
  $psql_superuser -c "DO \\$\$ BEGIN IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = '$DB_USER') THEN CREATE USER $DB_USER WITH PASSWORD '$DB_PASS'; ELSE ALTER USER $DB_USER WITH PASSWORD '$DB_PASS'; END IF; END \\$\$;" 2>&1 | head -5 || {
    log_warn "DO block failed, trying direct commands..."
    $psql_superuser -c "CREATE USER $DB_USER WITH PASSWORD '$DB_PASS';" 2>&1 | head -3 || true
    $psql_superuser -c "ALTER USER $DB_USER WITH PASSWORD '$DB_PASS';" 2>&1 | head -3 || true
  }

  # Create database if not exists
  log_db "Ensuring database '$DB_NAME' exists..."
  $psql_superuser -c "SELECT 'CREATE DATABASE $DB_NAME OWNER $DB_USER' WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = '$DB_NAME');\gexec" 2>&1 | head -5 || {
    $psql_superuser -c "CREATE DATABASE $DB_NAME OWNER $DB_USER;" 2>&1 | head -3 || true
  }

  # Grant privileges
  log_db "Granting privileges..."
  $psql_superuser -c "GRANT ALL PRIVILEGES ON DATABASE $DB_NAME TO $DB_USER;" 2>&1 | head -3 || true
...
deploy_api() {
  ...
  # Phase 2: Kill any remaining squatters
  kill_port_processes "$API_PORT" "API server"

  # Phase 3: Start new server in detached subshell
  cd "$API_DIR"
  > "$API_DIR/api.log"
  ...
  (
    ...
    # Wait inside subshell for port bind
  ...
  # Phase 4: Find the newly started process via port polling
  local new_pid=""
