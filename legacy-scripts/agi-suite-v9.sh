#!/usr/bin/env bash
#==============================================================================
# Agi-Suite — Full Stack Master Script v9
# Expert implementation: Fixed sudo variable expansion, peer auth detection
...
# Runs a command as the postgres superuser using peer auth (most reliable)
run_as_postgres() {
  local cmd="$1"
  local output

  # Method 1: sudo -u postgres psql (peer auth, no password needed)
  if output=$(sudo -u postgres psql -c "$cmd" 2>&1); then
    echo "$output"
    return 0
  fi

  # Method 2: sudo psql as postgres user
  if output=$(sudo psql "postgresql://postgres@localhost:$DB_PORT/postgres" -c "$cmd" 2>&1); then
    echo "$output"
    return 0
  fi

  # Method 3: direct psql as postgres (if pg_hba allows trust)
