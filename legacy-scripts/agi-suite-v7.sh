#!/usr/bin/env bash
#==============================================================================
# Agi-Suite — Full Stack Master Script v7
# Expert implementation: DB auto-provisioning, bulletproof deploy
# Triple-checked for race conditions and edge cases
#==============================================================================
...
get_port_pids() {
  ss -tlnp 2>/dev/null | grep "\b:$1\b" | grep -oP 'pid=\K[0-9]+' | sort -u | tr '\n' ' '
}
...
get_api_pid() {
  ss -tlnp 2>/dev/null | grep "\b:$API_PORT\b" | grep -oP 'pid=\K[0-9]+' | head -1
}
...
parse_db_url() {
  ...
  DB_USER=$(echo "$url" | sed -n 's|postgresql://\([^:]*\):.*||p')
  DB_PASS=$(echo "$url" | sed -n 's|postgresql://[^:]*:\([^@]*\)@.*||p')
  DB_HOST=$(echo "$url" | sed -n 's|postgresql://[^@]*@\([^:]*\):.*||p')
  DB_NAME=$(echo "$url" | sed -n 's|postgresql://.*/\(.*\)||p')
  DB_PORT_EXTRACT=$(echo "$url" | sed -n 's|postgresql://[^@]*@[^:]*:\([0-9]*\)/.*||p')
  [[ -n "$DB_PORT_EXTRACT" ]] && DB_PORT="$DB_PORT_EXTRACT"
}
...
check_db_connection() {
  ...
  if command -v psql &>/dev/null; then
    if psql "$DATABASE_URL" -c "SELECT 1 as connection_test;" 2>/dev/null | grep -q "connection_test"; then
      ...
      log_warn "Connection failed — user '$DB_USER' or database '$DB_NAME' may not exist"
      return 1
    fi
  fi
...
db_provision() {
  ...
  local superuser_url="postgresql://postgres@localhost:$DB_PORT/postgres"
  local can_superuser=0

  if psql "$superuser_url" -c "SELECT 1;" &>/dev/null; then
    can_superuser=1
  elif sudo -n psql "$superuser_url" -c "SELECT 1;" &>/dev/null; then
    can_superuser=1
    local use_sudo=1
  fi

  if [[ $can_superuser -eq 0 ]]; then
    log_warn "Cannot connect as postgres superuser — trying peer auth"
  ...
  if [[ $can_superuser -eq 0 ]]; then
    die "Cannot connect as postgres superuser to create user/database"
  fi

  # Create user if not exists
  log_db "Creating user '$DB_USER' if needed..."
  if [[ -n "${use_peer:-}" ]]; then
    sudo -u postgres psql -c "DO \$\$ BEGIN IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = '$DB_USER') THEN CREATE USER $DB_USER WITH PASSWORD '$DB_PASS'; END IF; END \$\$;" 2>/dev/null ||     sudo -u postgres psql -c "CREATE USER $DB_USER WITH PASSWORD '$DB_PASS';" 2>/dev/null ||     log_warn "User may already exist"
  elif [[ -n "${use_sudo:-}" ]]; then
    sudo psql "$superuser_url" -c "DO \$\$ BEGIN IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = '$DB_USER') THEN CREATE USER $DB_USER WITH PASSWORD '$DB_PASS'; END IF; END \$\$;" 2>/dev/null ||     sudo psql "$superuser_url" -c "CREATE USER $DB_USER WITH PASSWORD '$DB_PASS';" 2>/dev/null ||     log_warn "User may already exist"
  else
    psql "$superuser_url" -c "DO \$\$ BEGIN IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = '$DB_USER') THEN CREATE USER $DB_USER WITH PASSWORD '$DB_PASS'; END IF; END \$\$;" 2>/dev/null ||     psql "$superuser_url" -c "CREATE USER $DB_USER WITH PASSWORD '$DB_PASS';" 2>/dev/null ||     log_warn "User may already exist"
  fi

  # Create database if not exists
  log_db "Creating database '$DB_NAME' if needed..."
  if [[ -n "${use_peer:-}" ]]; then
    sudo -u postgres psql -c "SELECT 'CREATE DATABASE $DB_NAME OWNER $DB_USER' WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = '$DB_NAME');\gexec" 2>/dev/null ||     sudo -u postgres psql -c "CREATE DATABASE $DB_NAME OWNER $DB_USER;" 2>/dev/null ||     log_warn "Database may already exist"
  elif [[ -n "${use_sudo:-}" ]]; then
    sudo psql "$superuser_url" -c "SELECT 'CREATE DATABASE $DB_NAME OWNER $DB_USER' WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = '$DB_NAME');\gexec" 2>/dev/null ||     sudo psql "$superuser_url" -c "CREATE DATABASE $DB_NAME OWNER $DB_USER;" 2>/dev/null ||     log_warn "Database may already exist"
  else
    psql "$superuser_url" -c "SELECT 'CREATE DATABASE $DB_NAME OWNER $DB_USER' WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = '$DB_NAME');\gexec" 2>/dev/null ||     psql "$superuser_url" -c "CREATE DATABASE $DB_NAME OWNER $DB_USER;" 2>/dev/null ||     log_warn "Database may already exist"
  fi
  ...
  if [[ -n "${use_peer:-}" ]]; then
    ...
    sudo psql "$superuser_url" -c "GRANT ALL PRIVILEGES ON DATABASE $DB_NAME TO $DB_USER;" 2>/dev/null || true
  else
    psql "$superuser_url" -c "GRANT ALL PRIVILEGES ON DATABASE $DB_NAME TO $DB_USER;" 2>/dev/null || true
  fi
...
db_diagnose() {
  ...
  if [[ -n "${DATABASE_URL:-}" ]]; then
    parse_db_url
    log_db "Parsed: user='$DB_USER' host='$DB_HOST' port='$DB_PORT' db='$DB_NAME'"

    log_db "Testing connection with visible errors..."
    psql "$DATABASE_URL" -c "SELECT 1 as test;" 2>&1 | head -5 || true

    log_db "Listing databases (as postgres)..."
    psql "postgresql://postgres@localhost:$DB_PORT/postgres" -c "\l" 2>&1 | head -15 ||     sudo -u postgres psql -c "\l" 2>&1 | head -15 ||     log_warn "Cannot list databases"

    log_db "Checking users..."
    psql "postgresql://postgres@localhost:$DB_PORT/postgres" -c "\du" 2>&1 | head -10 ||     sudo -u postgres psql -c "\du" 2>&1 | head -10 ||     log_warn "Cannot list users"
  fi
}
...
db_migrate() {
  ...
  if ! check_db_connection; then
    log_warn "Database not accessible — attempting to provision..."
    db_provision
  fi

  cd "$DB_DIR"

  # Ensure DATABASE_URL is exported for drizzle
  if [[ -f "$API_DIR/.env" ]]; then
    set -a
    source "$API_DIR/.env"
    set +a
...
deploy_api() {
  ...
  # Phase 1: Stop existing tracked process
  if [[ -f "$PID_FILE" ]]; then
    local old_pid
  ...
  # Phase 2: Kill any remaining squatters on the port
  kill_port_processes "$API_PORT" "API server"

  # Phase 3: Start new server
  cd "$API_DIR"
  > "$API_DIR/api.log"

  # CRITICAL: Use a completely detached process via nohup in a subshell
  # The subshell exits immediately, leaving only the nohup'd node process
  (
    cd "$API_DIR"
    nohup node --env-file=.env --enable-source-maps ./dist/index.mjs >> api.log 2>&1 &
    # Wait inside subshell for node to bind
    local inner_attempts=0
    while [[ $inner_attempts -lt 30 ]]; do
      sleep 0.3
      if ss -tlnp 2>/dev/null | grep -q "\b:$API_PORT\b"; then
        break
      fi
      ((inner_attempts++))
    done
    # Subshell exits here — node continues in background
  )

  # Phase 4: Find the newly started process (must be different from any old PIDs)
  local new_pid=""
  ...
  # Phase 5: Wait for health endpoint
...
deploy_fe() {
  ...
  (
    ...
    while [[ $inner_attempts -lt 30 ]]; do
      sleep 0.3
      if ss -tlnp 2>/dev/null | grep -q "\b:$FE_PORT\b"; then
        break
      fi
      ((inner_attempts++))
    done
  ...
  while [[ -z "$new_pid" ]] && [[ $outer_attempts -lt 30 ]]; do
    sleep 0.3
    new_pid=$(ss -tlnp 2>/dev/null | grep "\b:$FE_PORT\b" | grep -oP 'pid=\K[0-9]+' | head -1)
    ((outer_attempts++))
  done
...
stack_status() {
  ...
  local fe_line
  fe_line=$(ss -tlnp 2>/dev/null | grep -E ':517[3-9] ' | head -1)
  if [[ -n "$fe_line" ]]; then
    local fe_port
    fe_port=$(echo "$fe_line" | awk '{print $4}' | cut -d: -f2)
    local fe_pid
    fe_pid=$(echo "$fe_line" | grep -oP 'pid=\K[0-9]+' | head -1)
...
show_api_status() {
  ...
  if [[ -n "$pid_from_file" ]] && kill -0 "$pid_from_file" 2>/dev/null; then
    ...
    if [[ -n "$pid_from_port" ]]; then
      ss -tlnp | grep "\b:$API_PORT\b" || true
    fi
    return 0
  fi
...
DATABASE COMMANDS:
  ...
  db-provision Auto-create user/database if missing
...
Examples:
  ...
  ./agi-suite.sh db-provision  # Create missing user/db
