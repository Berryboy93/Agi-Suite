#!/usr/bin/env bash
#==============================================================================
# Agi-Suite — Full Stack Master Script v13
# Clean rewrite: all fixes applied, syntax verified
#==============================================================================
...
get_api_pid() {
  ss -tlnp 2>/dev/null | awk -v p=":"API_PORT" " '$4 ~ p {print}' | \
...
deploy_fe() {
  ...
  while [[ -z "$new_pid" ]] && [[ $outer_attempts -lt 30 ]]; do
    sleep 0.3
    new_pid=$(ss -tlnp 2>/dev/null | awk -v p=":"FE_PORT" " '$4 ~ p {print}' | \
      sed -n 's/.*pid=\([0-9]*\).*/\1/p' | head -1)
    outer_attempts=$((outer_attempts + 1))
  done
