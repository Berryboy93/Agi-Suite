#!/usr/bin/env bash
#==============================================================================
# Agi-Suite — Full Stack Master Script v6
# Fixes: DB diagnostics, deploy with proper backgrounding, port bind polling
#==============================================================================
...
db_diagnose() {
  ...
  if ! check_db; then
    die "Database is not running"
  fi

  log_db "DATABASE_URL: ${DATABASE_URL:-NOT SET}"

  if [[ -n "${DATABASE_URL:-}" ]]; then
    log_db "Testing connection..."
    psql "$DATABASE_URL" -c "\conninfo" 2>/dev/null || log_warn "Connection info failed"

    log_db "Listing databases..."
    psql "$DATABASE_URL" -c "\l" 2>/dev/null | head -10 || log_warn "Cannot list databases"

    log_db "Checking schema..."
    psql "$DATABASE_URL" -c "SELECT schema_name FROM information_schema.schemata;" 2>/dev/null | head -10 || log_warn "Cannot query schemata"

    log_db "Checking tables..."
    psql "$DATABASE_URL" -c "SELECT table_name FROM information_schema.tables WHERE table_schema='public';" 2>/dev/null | head -10 || log_warn "Cannot query tables"
  fi

  log_db "Drizzle config:"
  cat "$DB_DIR/drizzle.config.ts" 2>/dev/null || log_warn "No drizzle config"
}
...
db_migrate() {
  ...
  log_db "DATABASE_URL is: ${DATABASE_URL:-NOT SET}"

  # Try push with verbose output
  log_db "Running drizzle-kit push..."
  pnpm push 2>&1 || {
    log_error "Migration failed — checking schema..."
    db_diagnose
...
db_status() {
  if check_db; then
    if [[ -n "${DATABASE_URL:-}" ]] && command -v psql &>/dev/null; then
      ...
      log_db "DATABASE_URL not available for connection check"
    fi
  fi
}
...
deploy_api() {
  ...
  # Start with proper detachment — use a subshell that exits immediately
  # This prevents the parent shell from tracking the job
  (
    cd "$API_DIR"
    exec node --env-file=.env --enable-source-maps ./dist/index.mjs >> api.log 2>&1
  ) &

  # Immediately disown to remove from shell job table
  disown 2>/dev/null || true

  # Wait for node to actually bind to the port
  local node_pid=""
  local attempts=0
  while [[ -z "$node_pid" ]] && [[ $attempts -lt 30 ]]; do
    sleep 0.5
...
deploy_fe() {
  ...
  # Start Vite with proper detachment
  (
    cd "$FE_DIR"
    exec pnpm dev >> frontend.log 2>&1
  ) &
  ...
  while [[ -z "$fe_pid" ]] && [[ $attempts -lt 30 ]]; do
    sleep 0.5
    # Check expected port first
    ...
    local fallback
    fallback=$(ss -tlnp 2>/dev/null | grep -E ':517[3-9] ' | head -1)
    if [[ -n "$fallback" ]]; then
      actual_port=$(echo "$fallback" | awk '{print $4}' | cut -d: -f2)
      fe_pid=$(echo "$fallback" | grep -oP 'pid=\K[0-9]+' | head -1)
      break
    fi
    ((attempts++))
  done
  ...
  if [[ "$actual_port" != "$FE_PORT" ]]; then
    log_warn "Vite using port $actual_port instead of $FE_PORT"
    FE_PORT="$actual_port"
    FE_URL="http://localhost:$FE_PORT"
  fi
...
DATABASE COMMANDS:
  ...
  db-diagnose Run full DB diagnostics
...
Examples:
  ...
  ./agi-suite.sh db-diagnose  # Debug DB issues
