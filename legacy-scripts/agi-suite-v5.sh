#!/usr/bin/env bash
#==============================================================================
# Agi-Suite — Full Stack Master Script v5
# Fixes: DB env loading, deploy race condition, fe-deploy hang
#==============================================================================
...
if [[ -f "$API_DIR/.env" ]]; then
  export $(grep -v '^#' "$API_DIR/.env" | xargs) 2>/dev/null || true
fi
...
deploy_fe() {
  ...
  while [[ -z "$fe_pid" ]] && [[ $attempts -lt 20 ]]; do
    sleep 0.5
