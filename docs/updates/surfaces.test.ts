...
describe("lookupBlastRadius — dev-build-isolated", () => {
  ...
  it("config_change = low", () => {
    expect(lookupBlastRadius("dev-build-isolated", "config_change")).toBe(
      "low",
    );
  });
  it("deploy = INVALID", () => {
...
describe("lookupBlastRadius — supply chain", () => {
  it("dependency_update = high", () => {
    expect(
      lookupBlastRadius("dev-build-supply-chain", "dependency_update"),
    ).toBe("high");
  });
  it("code_change = INVALID", () => {
    expect(lookupBlastRadius("dev-build-supply-chain", "code_change")).toBe(
      INVALID,
    );
  });
});
