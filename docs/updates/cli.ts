...
function defaultBarrierSnapshot(): BarrierSnapshot {
  ...
  return {
    cryptographicAuth: {
      ...
      active: false,
      checkedAt: now,
      reason: 'No infrastructure health check provided — defaulting to inactive (safe-fail)',
    },
...
async function main(): Promise<void> {
  ...
  if (parsed.barriers) {
    try {
      ...
      console.error(`[r3-governance] Loaded barrier snapshot from: ${parsed.barriers}`);
    } catch (err) {
      console.error(
  ...
  if (mythosResult.flaggedForManualReview) {
    console.error('[r3-governance] ⚠️  FLAGGED FOR MANUAL REVIEW — anchoring divergence detected.');
  }
  ...
  console.error('[r3-governance] ✅ Exit 0: All pairs within acceptable thresholds.');
  process.exit(0);
}
