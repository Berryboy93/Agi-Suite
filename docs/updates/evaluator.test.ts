...
import { evaluateChangeRequest, type ChangeRequest } from '../src/rules/evaluator.js';
...
function makeBarriers(overrides: Partial<BarrierSnapshot> = {}): BarrierSnapshot {
  const base: BarrierSnapshot = {
    cryptographicAuth: { id: 'cryptographicAuth', active: true, checkedAt: NOW },
    credentialVaultIsolation: { id: 'credentialVaultIsolation', active: true, checkedAt: NOW },
    ...
    productionDeploymentGate: { id: 'productionDeploymentGate', active: true, checkedAt: NOW },
  };
...
const NO_BARRIERS_ACTIVE = makeBarriers({
  cryptographicAuth: { id: 'cryptographicAuth', active: false, checkedAt: NOW, reason: 'test' },
  credentialVaultIsolation: { id: 'credentialVaultIsolation', active: false, checkedAt: NOW, reason: 'test' },
  paymentIsolation: { id: 'paymentIsolation', active: false, checkedAt: NOW, reason: 'test' },
  sandboxExecution: { id: 'sandboxExecution', active: false, checkedAt: NOW, reason: 'test' },
  productionDeploymentGate: { id: 'productionDeploymentGate', active: false, checkedAt: NOW, reason: 'test' },
});
  ...
  it('runtime + auth_change with valid defer structure → DEFER', () => {
    const req = makeRequest(
      [{ surface: 'runtime', actionType: 'auth_change' }],
      {
        ...
        interimControl: 'JWT tokens rotated, MFA enforced on all admin accounts',
      },
    );
    const result = evaluateChangeRequest(req, ALL_BARRIERS_ACTIVE);
...
describe('Rule 4 — medium blast radius', () => {
  it('runtime + code_change (medium) + sandboxExecution active → ALLOW_SANDBOX', () => {
    const req = makeRequest([{ surface: 'runtime', actionType: 'code_change' }]);
    const result = evaluateChangeRequest(req, ALL_BARRIERS_ACTIVE);
    expect(result.finalOutcome).toBe('ALLOW_SANDBOX');
  });
});
    ...
    const req = makeRequest([
      {
        ...
        repricingRationale: 'Scope limited to static asset only; no runtime code path affected',
      },
    ]);
    const result = evaluateChangeRequest(req, ALL_BARRIERS_ACTIVE);
    ...
    const req = makeRequest([
      {
        surface: 'runtime',
        actionType: 'deploy', // high → ALLOW_STAGING
        advisory: { label: 'critical', cvssScore: 9.8, mappedLevel: 'critical' },
        repricedLevel: 'low', // gap of 3 levels — exceeds threshold
        repricingRationale: 'Attempting suspicious downgrade',
      },
    ]);
    const result = evaluateChangeRequest(req, ALL_BARRIERS_ACTIVE);
    expect(result.pairResults[0]!.repricing.anchoringDivergenceDetected).toBe(true);
