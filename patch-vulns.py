...
Usage:
    # From Agi-Suite directory (uses ./package.json by default):
    python3 patch-vulns.py
    python3 patch-vulns.py --apply
    
    # Explicit path:
    python3 patch-vulns.py /path/to/Agi-Suite/package.json --apply
"""
...
def main():
    ...
    # Extract path (filter out flags)
    path_args = [arg for arg in args if not arg.startswith("--")]
    
    if path_args:
        pkg_path = Path(path_args[0])
    else:
        # Default to ./package.json if running from repo root
        pkg_path = Path("./package.json")
    
    if not pkg_path.exists():
        print(f"ERROR: {pkg_path} not found", file=sys.stderr)
        print(f"\nUsage: python3 patch-agi-suite-vulns.py [./package.json] [--apply]")
        sys.exit(1)
