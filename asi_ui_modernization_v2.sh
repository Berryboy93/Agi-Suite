#!/usr/bin/env bash
# ASI Expert-Level UI Modernization Script v2
...
echo "═══════════════════════════════════════════════════════════════"
echo "  ASI Expert UI Modernization — Phase 1 Completion (v2)"
echo "═══════════════════════════════════════════════════════════════"
echo ""
...
# ═══════════════════════════════════════════════════════════════
# STEP 4: Revert ALL Relative Imports → Alias Imports
# ═══════════════════════════════════════════════════════════════
echo ">>> STEP 4: Revert Relative Imports → Clean Alias Imports"

# Handle ALL relative depth patterns that point to ui/
find "$APP_DIR/src" -name "*.tsx" -type f | while read -r file; do
    # Pattern: ../../ui/* (from views/)
    sed -i 's|"../../ui/|"@/ui/|g' "$file"
    sed -i "s|'../../ui/|'@/ui/|g" "$file"

    # Pattern: ../ui/* (from components/views/ or similar)
    sed -i 's|"../ui/|"@/ui/|g' "$file"
    sed -i "s|'../ui/|'@/ui/|g" "$file"

    # Pattern: ./ui/* (unlikely but handle defensively)
    sed -i 's|"./ui/|"@/ui/|g' "$file"
    sed -i "s|'./ui/|'@/ui/|g" "$file"
done
...
- Convert all relative ../ui/, ../../ui/, and ./ui/ imports to @/ui/*
