...
import { AgentSuitePanel, AgentSuiteToggle } from './components/AgentSuitePanel';
...
function BootScreen({ onDone }: { onDone: () => void }) {
  ...
  useEffect(() => {
    ...
    BOOT_LINES.forEach((_, i) => {
      timers.push(setTimeout(() => setVisibleLines(prev => [...prev, i]), i * 450 + 100));
    });
  ...
  return (
    <div style={{
      ...
      <div style={{ fontFamily: 'var(--sans)', fontSize: 13, fontWeight: 800, color: 'var(--acid)', letterSpacing: 3, marginBottom: 16, textTransform: 'uppercase' }}>
        ...
        {BOOT_LINES.map((line, i) => (
          <div
            ...
            style={{
              fontFamily: 'var(--mono)',
              fontSize: 11,
              color: i === BOOT_LINES.length - 1 ? 'var(--done)' : 'var(--text2)',
              letterSpacing: 0.5,
...
function CenterContent() {
  ...
  return (
    <main style={{ overflowY: 'auto', padding: 16, display: 'flex', flexDirection: 'column', gap: 14 }}>
      {focusBanner && (
        <div style={{ padding: '6px 13px', background: 'rgba(163,230,53,.06)', borderBottom: '1px solid rgba(163,230,53,.15)', fontSize: 10, color: 'var(--acid)', letterSpacing: 1, display: 'flex', alignItems: 'center', gap: 8, borderRadius: 3 }}>
          <span style={{ fontSize: 12 }}>◈</span>
          <span>{focusBanner}</span>
          <span onClick={clearFocus} style={{ marginLeft: 'auto', cursor: 'pointer', color: 'var(--dim)', fontSize: 11 }}>✕</span>
        </div>
      )}
...
function App() {
  ...
  return (
    <>
      ...
      >
        <Header />
        <div style={{ display: 'grid', gridTemplateColumns: '224px 1fr 304px', height: 'calc(100vh - 50px)', overflow: 'hidden' }}>
          <Sidebar />
          <CenterContent />
          <RightPanel />
        </div>
      ...
      <AgentSuiteToggle
        isOpen={agentSuiteOpen}
        onClick={toggleAgentSuite}
      />
      <AgentSuitePanel
        isOpen={agentSuiteOpen}
        onClose={toggleAgentSuite}
      />
    </>
  );
}
